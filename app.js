...
const STATE = {
  ...
  // Trial Limits for Non-Verified Keys
  limits: {
    chatQueriesUsed: 0,
    chatQueryLimit: 3,
    summaryGenerationsUsed: 0,
    summaryGenerationLimit: 1
  }
};
...
function generateInteractiveMindMap() {
  ...
  const isVerified = STATE.api.key && !STATE.api.demoMode;
...
async function sendUserChatMessage() {
  ...
  const isVerified = STATE.api.key && !STATE.api.demoMode;
  if (!isVerified) {
    if (STATE.limits.chatQueriesUsed >= STATE.limits.chatQueryLimit) {
      appendChatBubble('system', `⚠️ **Query Limit Reached (Demo Mode)**\n\nYou have reached the limit of **${STATE.limits.chatQueryLimit}** queries in Demo Mode.\n\nTo unlock unlimited queries and chat with your documents, please click the **Gemini Config** button on the top right and enter your Gemini API Key.`);
      return;
    }
  }
...
async function generateWorkspaceSummary() {
  ...
  const isVerified = STATE.api.key && !STATE.api.demoMode;
  if (!isVerified) {
    if (STATE.limits.summaryGenerationsUsed >= STATE.limits.summaryGenerationLimit) {
      output.innerHTML = `
        <div class="tool-empty-state">
          <svg class="tool-illustration-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="color: var(--accent-rose);">
            <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
            <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
          </svg>
          <h5 style="color: var(--accent-rose);">Synthesis Limit Reached</h5>
          <p>You have reached the limit of **${STATE.limits.summaryGenerationLimit}** summary generation in Demo Mode.</p>
          <p style="font-size:0.75rem; margin-top:10px; color: var(--text-tertiary);">Please click the **Gemini Config** button on the top right and enter your Gemini API Key to unlock unlimited workspace summaries.</p>
        </div>
      `;
      return;
    }
  ...
  try {
    ...
    if (!isVerified) {
      STATE.limits.summaryGenerationsUsed++;
    }
  } catch (err) {
    output.innerHTML = `
