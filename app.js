...
const STATE = {
  ...
  api: {
    ...
    isValid: localStorage.getItem('york_api_key_valid') === 'true'
  },
  ...
  // Trial Limits for Non-Verified Keys (Unified 5 free attempts)
  limits: {
    totalAttemptsUsed: parseInt(localStorage.getItem('york_attempts_used')) || 0,
    freeLimit: 5
  }
};
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
// Global Object URL tracker for native PDF rendering
let activePdfObjectUrl = null;

function setActiveDocument(source) {
  // If switching away or clearing, clean up current Object URL to save memory
  if (activePdfObjectUrl && (!source || source.id !== STATE.activeDoc?.id)) {
  ...
  if (!source) {
    ...
    const pageCountBadge = document.getElementById('reader-page-count');
    if (pageCountBadge) pageCountBadge.style.display = 'none';
    ...
    if (STATE.activeDoc.dataUrl) {
      ...
      let htmlContent = STATE.activeDoc.text
        ...
        .map(line => {
          ...
          // Check if it's a page indicator like [Page X]
          const pageMatch = trimmed.match(/^\[Page\s+(\d+)\]$/i);
          if (pageMatch) {
            return `
              <div class="reader-page-divider">
                <span class="reader-page-line"></span>
                <span class="reader-page-badge">Page ${pageMatch[1]}</span>
                <span class="reader-page-line"></span>
              </div>
            `;
          }
      ...
      // Update Reader toolbar page count badge
      const pageCountBadge = document.getElementById('reader-page-count');
      if (pageCountBadge) {
        if (STATE.activeDoc.format === 'pdf') {
          const matches = STATE.activeDoc.text.match(/\[Page\s+\d+\]/gi);
          const total = matches ? matches.length : 1;
          pageCountBadge.innerText = `${total} Pages`;
...
// Native PDF Iframe renderer (gives full Adobe Acrobat options like zoom, pages, search, print, download)
async function renderOriginalPDFPages() {
  ...
  container.innerHTML = '<div class="loader-spinner">Loading original PDF document...</div>';
  
  try {
    let pdfBlob;
    
    // Check if the source has fileBlob
    if (STATE.activeDoc.fileBlob) {
      pdfBlob = STATE.activeDoc.fileBlob;
    } else {
      // Fallback: If not in memory, we fetch the source again from DB
      const transaction = STATE.db.transaction(['sources'], 'readonly');
      const store = transaction.objectStore('sources');
    ...
    // Append #toolbar=1 to show Acrobat navigation toolbar
    const pdfUrl = `${activePdfObjectUrl}#toolbar=1&navpanes=1`;
    
    container.innerHTML = `
      <iframe src="${pdfUrl}" class="pdf-iframe-viewer" title="${escapeHTML(STATE.activeDoc.title)}"></iframe>
    `;
  } catch (err) {
    console.error('Error loading PDF in iframe:', err);
    container.innerHTML = `
      <div style="color: var(--accent-rose); padding: 32px; text-align: center;">
        <h5>Failed to load PDF document</h5>
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
function generateInteractiveMindMap(isExplicitClick = false) {
  try {
    ...
    const isVerified = STATE.api.key && STATE.api.isValid;
    if (isExplicitClick && !isVerified && checkTrialLimitReached('Mind Map Concept Mapping')) {
      return;
    }
    
    let sourcesToMap = activeSources;
    
    if (!isVerified) {
      // Limit to exactly the first active source
      sourcesToMap = [activeSources[0]];
      docCountText.innerHTML = `1 source mapped <span style="color: #dc2626; font-weight: 600; margin-left: 6px;">(Demo Limit - 1 Max)</span>`;
    } else {
      docCountText.innerText = `${activeSources.length} source${activeSources.length > 1 ? 's' : ''} mapped`;
    }
    
    if (isExplicitClick && !isVerified) {
      STATE.limits.totalAttemptsUsed++;
      localStorage.setItem('york_attempts_used', STATE.limits.totalAttemptsUsed);
    }
    ...
    // 2. Add Topic Nodes (representing each mapped source)
    const angleStep = (2 * Math.PI) / sourcesToMap.length;
    
    sourcesToMap.forEach((src, idx) => {
      ...
      const topicNode = {
        ...
        r: 9,
        description: `Source document containing ${(src.text || '').split(' ').length} words. Added on ${src.addedAt ? new Date(src.addedAt).toLocaleDateString() : 'N/A'}.`,
        sourceId: src.id
      };
      ...
      const subangleStep = concepts.length > 0 ? (2 * Math.PI) / concepts.length : 0;
    ...
    if (container) {
      container.innerHTML = `
        <div style="color: var(--accent-rose); padding: 32px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%;">
          <h5>Failed to generate concept map</h5>
          <p style="font-size:0.8rem; color:var(--text-secondary); margin-top:8px; max-width: 320px;">${err.message || err}</p>
        </div>
      `;
    }
  }
...
// Smart NLP helper to extract concepts, summary points, and quotes from local document text
function extractDocumentInsights(text, title) {
  const safeText = text || '';
  if (!safeText || safeText.trim().length === 0) {
    return {
      concepts: [
        { term: 'No Sources Mapped', def: 'Add and check a source document in the left pane to start building your conceptual map.' },
        { term: 'Collaborative Notes', def: 'Add custom text notes on the workspace notes tab to provide manual contexts.' },
        { term: 'Getting Started', def: 'Drag-and-drop a PDF, Word document, or markdown file here.' }
      ],
      executiveSummary: 'This workspace contains no active documents or notes. Please upload files to see dynamic synthesis summaries.',
      keyPoints: ['No source content loaded.'],
      conclusions: ['No conclusions derived.']
    };
  }
  ...
  const sentences = safeText
    ...
    .filter(s => s.length > 15 && s.length < 300 && !s.includes('pdf.worker.min.js') && !s.includes('CDNs'));

  // Split text into words and compute frequencies to find key terms
  const stopWords = new Set(['the', 'and', 'a', 'of', 'to', 'in', 'is', 'that', 'for', 'it', 'on', 'with', 'as', 'this', 'was', 'by', 'an', 'be', 'are', 'from', 'at', 'or', 'your', 'we', 'our', 'you', 'not', 'have', 'has', 'but', 'which', 'about', 'more', 'their', 'there', 'they', 'what', 'who', 'will', 'would', 'can', 'should', 'all', 'any', 'each', 'other', 'some', 'than', 'them', 'then', 'into', 'only', 'out', 'up', 'its', 'no', 'so', 'if']);
  
  // Find words of length 4 to 12
  const words = safeText.toLowerCase().match(/\b[a-z]{4,12}\b/g) || [];
  const freq = {};
  words.forEach(w => {
    if (!stopWords.has(w)) {
      freq[w] = (freq[w] || 0) + 1;
  ...
  // Find key concepts: match terms with sentences that sound like descriptions/definitions
  const concepts = [];
  const seenTerms = new Set();

  // 1. Try to find descriptive sentences containing top terms
  for (const term of sortedTerms) {
    if (concepts.length >= 3) break;
    const definitionSentence = sentences.find(s => {
      const lower = s.toLowerCase();
      // Match sentences like: "concept is...", "term refers to...", "term means...", "term focus...", "are..."
      return lower.includes(term) && (lower.includes(' is ') || lower.includes(' are ') || lower.includes(' means ') || lower.includes(' refers ') || lower.includes(' focus ') || lower.includes(' process ') || lower.includes(' analysis '));
    });
    ...
    if (definitionSentence) {
      ...
      concepts.push({
        ...
        def: definitionSentence.length > 140 ? definitionSentence.substring(0, 137) + '...' : definitionSentence
      });
      seenTerms.add(term);
    }
  }

  // 2. Fallback to just regular sentences containing top terms if we don't have 3 concepts
  for (const term of sortedTerms) {
    if (concepts.length >= 3) break;
    ...
    if (sentence) {
      ...
      concepts.push({
        term: capitalizedTerm,
        def: sentence.length > 140 ? sentence.substring(0, 137) + '...' : sentence
      });
      seenTerms.add(term);
    }
  }

  // 3. Fallback to standard baseline if still not enough concepts
  if (concepts.length === 0) {
    concepts.push(
      { term: 'Core Themes', def: sentences[0] || 'Primary repeating arguments running through the workspace source documents.' },
      { term: 'Analytical Focus', def: sentences[Math.floor(sentences.length / 2)] || 'Main analytical premises and systematic details discussed.' },
      { term: 'Conclusions', def: sentences[sentences.length - 1] || 'Key summaries and deducing points outlined.' }
    );
  }

  // Executive summary: take first 2-3 sentences of the text
  const executiveSummaryText = sentences.slice(0, 3).join('. ') + '.';
  ...
  for (let i = 1; i <= 3; i++) {
    const idx = i * step;
    if (idx < sentences.length && sentences[idx]) {
      keyPoints.push(sentences[idx]);
    }
  }
  if (keyPoints.length === 0) {
    keyPoints.push(sentences[0] || 'No content parsed.');
  }

  // Conclusions
  const conclusions = sentences.slice(-2).map(s => s.trim());
  if (conclusions.length === 0) {
    conclusions.push('End of document content.');
  }

  return { concepts, executiveSummary: executiveSummaryText, keyPoints, conclusions };
}

// Simple deterministic concept extractor to generate rich maps offline
function extractConceptsFromText(text, filename) {
  const insights = extractDocumentInsights(text, filename);
...
function setupMindMapViewportControls() {
  ...
  const maximizeBtn = document.getElementById('mindmap-maximize-btn');
  if (maximizeBtn) {
    maximizeBtn.onclick = () => {
      const workspace = document.querySelector('.mindmap-workspace');
      workspace.classList.toggle('maximized');
      
      const isMaximized = workspace.classList.contains('maximized');
      maximizeBtn.title = isMaximized ? 'Exit Fullscreen' : 'Toggle Fullscreen';
      
      const svg = maximizeBtn.querySelector('svg');
      if (svg) {
        if (isMaximized) {
          svg.innerHTML = '<path d="M4 14h6v6M20 10h-6V4M14 10l7-7M10 14l-7 7" stroke="currentColor" stroke-width="2"/>';
        } else {
          svg.innerHTML = '<path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" stroke="currentColor" stroke-width="2"/>';
        }
      }
      
      STATE.mindmap.zoom = { x: 0, y: 0, scale: isMaximized ? 1.3 : 1.0 };
      updateMindMapTransform();
    };
  }
...
// Silent API Key validation helper
async function validateAPIKeySilently(key) {
  if (!key) return false;
  
  // 1. Lenient length check (Google API keys are at least 20 chars, official keys start with AIzaSy)
  const looksLikeGoogleKey = key.length >= 20;
  ...
  try {
    ...
    // Check if the API explicitly rejects the key itself as invalid
    if (lowerMsg.includes('api key not valid') || lowerMsg.includes('api_key_invalid') || lowerMsg.includes('key not valid') || lowerMsg.includes('invalid api key')) {
      if (lowerMsg.includes('model')) {
        return true; // Key itself is valid, but restricted for this specific model
      }
      return false;
    }
    
    // Other API errors (such as quota restrictions, model limits, region blocks) indicate the key itself is valid credentials
    return true;
  } catch (e) {
    // Network connectivity error (CORS block on local file, proxy or offline state)
    // Fall back to format check so users are not locked out of their trials
    return looksLikeGoogleKey;
  }
}

// Unified trial limit helper
function checkTrialLimitReached(actionName) {
  const isVerified = STATE.api.key && STATE.api.isValid;
  if (!isVerified && STATE.limits.totalAttemptsUsed >= STATE.limits.freeLimit) {
    const configModal = document.getElementById('api-config-modal');
    if (configModal) configModal.classList.remove('hidden');
    
    alert(`⚠️ Trial Limit Reached\n\nYou have used your ${STATE.limits.freeLimit} free trial attempts.\n\nPlease enter a verified Gemini API Key to continue using Chat, Summarise, Presentation, and Mind Map mapping.`);
    return true;
  }
  return false;
}
...
async function simulateMockAIResponse(prompt, systemInstruction, streamCallback) {
  ...
  // Combine all active sources texts + notes
  let combinedText = activeSources.map(s => s.text).join('\n\n');
  if (STATE.notes.text.trim()) {
    combinedText += '\n\n' + STATE.notes.text;
  }
  
  const insights = extractDocumentInsights(combinedText, titles);
  let responseText = '';
  
  // Differentiate response based on action / prompt
  if (prompt.includes('__SYNTHESIZE_SUMMARY__')) {
    const conceptBullets = insights.concepts.map(c => `- **${c.term}**: ${c.def}`).join('\n');
    const pointsBullets = insights.keyPoints.map(kp => `- ${kp}`).join('\n');
    const conclusionBullets = insights.conclusions.map(c => `- ${c}`).join('\n');
    
    responseText = `### Executive Research Synthesis: Workspace Context
This synthesis encapsulates local analysis across your active workspace documents (${titles || 'Workspace Notes'}).

#### 1. Executive Summary
- **Core Objectives**: ${insights.executiveSummary}
...
- **Workspace Context**: This outline was synthesized directly from the textual structure of **${titles || 'Workspace Notes'}**.
${conclusionBullets}

*(Note: yOrk LLM is currently running in Demo Mode. Connect your Gemini API Key in the settings panel to enable advanced semantic reasoning across all document pages!)*`;
  } 
  else if (prompt.includes('__BUILD_PRESENTATION__')) {
    const slide1Bullets = insights.keyPoints.map(kp => `- ${kp.length > 80 ? kp.substring(0, 77) + '...' : kp}`).slice(0, 3).join('\n');
    const slide2Bullets = insights.concepts.map(c => `- ${c.term}: ${c.def.length > 80 ? c.def.substring(0, 77) + '...' : c.def}`).join('\n');
    const slide3Bullets = insights.conclusions.map(c => `- ${c.length > 80 ? c.substring(0, 77) + '...' : c}`).slice(0, 3).join('\n');

    responseText = `[SLIDES]
---
[SLIDE 1]
Title: Research Briefing: ${activeSources[0]?.title || 'Workspace'}
Subtitle: Automated workspace insights from sources
Bullets:
${slide1Bullets || '- No content parsed.'}
---
[SLIDE 2]
Title: Core Vocabulary & Concepts
Subtitle: Key terms extracted locally
Bullets:
${slide2Bullets || '- No concepts extracted.'}
---
[SLIDE 3]
Title: Findings & Conclusions
Subtitle: Key takeaways from document text
Bullets:
${slide3Bullets || '- No conclusions.'}
[/SLIDES]

[FAQ]
**Q1: What is the primary focus of this document?**
*A1:* The document primarily outlines: ${insights.executiveSummary.substring(0, 150)}...

**Q2: What are the main terms defined in this workspace?**
*A2:* The central concepts are **${insights.concepts.map(c => c.term).join(', ')}**.
[/FAQ]`;
  } 
  else {
...
*Note: You are currently running in **Demo Mode (Mock replies)**. To connect real Gemini Flash 2.5 models, enter your key in the top right dialog.*`;
      ...
      const queryWords = prompt.toLowerCase().match(/\b[a-z]{3,15}\b/g) || [];
      let bestMatches = [];
      
      // Look for sentences that contain the query words
      const allSentences = combinedText
        .split(/[.!?]+/)
        .map(s => s.trim().replace(/\s+/g, ' '))
        .filter(s => s.length > 15 && s.length < 300 && !s.includes('pdf.worker.min.js'));
        
      allSentences.forEach(s => {
        let matches = 0;
        const lowerS = s.toLowerCase();
        queryWords.forEach(qw => {
          if (lowerS.includes(qw)) matches++;
        });
        if (matches > 0) {
          bestMatches.push({ sentence: s, matches });
        }
      });
      
      // Sort matches by relevance
      bestMatches.sort((a, b) => b.matches - a.matches);
      
      if (bestMatches.length > 0) {
        const topQuotes = bestMatches.slice(0, 2).map((m, idx) => `- *"${m.sentence}."* [${idx + 1}]`).join('\n');
        responseText = `Based on your active source document(s) (${titles || 'Workspace Notes'}), here is a detailed breakdown matching your query:

1. **Relevance in Context**: The document text directly discusses this topic. Keyword matching identified the following relevant excerpts.
2. **Key Excerpts & Quotations**:
${topQuotes}
3. **Local Synthesis**:
   - The document explicitly mentions the relationships between these keywords.
   - For a deeper semantic analysis, connect your Gemini API Key in the configuration modal.

*(Note: yOrk LLM is currently running in Demo Mode. Connect your Gemini API Key in the settings panel to enable advanced semantic reasoning across all document pages!)*`;
      } else {
        responseText = `I searched your active sources (${titles || 'Workspace Notes'}) for keyword matches about "${prompt}" but could not find a direct quotation. 

Here is an excerpt from the beginning of your active document (**${activeSources[0]?.title || 'Workspace Notes'}**):

> "${combinedText.substring(0, 300).trim()}..."

Is there another topic or specific keyword you would like me to search for?

*(Note: yOrk LLM is currently running in Demo Mode. Connect your Gemini API Key in the settings panel to enable advanced semantic search across all pages!)*`;
      }
    }
  }
...
async function sendUserChatMessage() {
  ...
  const isVerified = STATE.api.key && STATE.api.isValid;
  if (!isVerified && checkTrialLimitReached('Chat Query')) {
    return;
  }
  ...
  try {
    ...
    systemPrompt = `You are yOrk LLM, an expert document intelligence assistant.
...
${ragContext}`;
    ...
    if (isQuotaError) {
      try {
        console.warn('Quota exceeded detected, falling back to simulated chat response...');
        const fallbackQuery = query || 'Hello';
        const fallbackSystemPrompt = systemPrompt || `You are yOrk LLM.`;
        
        const mockResult = await simulateMockAIResponse(fallbackQuery, fallbackSystemPrompt, (partialText) => {
          let parsedHtml = marked.parse(partialText);
          bubbleBody.innerHTML = formatCitations(parsedHtml);
        });
        ...
        warningDiv.style.backgroundColor = 'rgba(220, 38, 38, 0.1)';
        warningDiv.style.borderLeft = '3px solid #dc2626';
        warningDiv.style.borderRadius = '4px';
        warningDiv.style.fontSize = '0.7rem';
        warningDiv.style.color = 'var(--text-primary)';
        warningDiv.innerHTML = `<strong>⚠️ Google API Quota Exceeded</strong><br>Your Gemini API key ran out of quota. Generated a simulated reply offline so you are not blocked.`;
        bubbleBody.appendChild(warningDiv);
        
        const viewport = document.getElementById('chat-logs-viewport');
        viewport.scrollTop = viewport.scrollHeight;
        return;
      } catch (mockErr) {
        console.error('Chat fallback simulation failed:', mockErr);
      }
    }
...
async function generateWorkspaceSummary() {
  ...
  const isVerified = STATE.api.key && STATE.api.isValid;
  if (!isVerified && checkTrialLimitReached('Document Synthesis')) {
    return;
  }
  ...
  try {
    const ragContext = getRAGContext();
    systemPrompt = `You are a research synthesis AI. Write a comprehensive summary based strictly on the provided documents.

${ragContext}`;
    query = `__SYNTHESIZE_SUMMARY__ Extract executive summary, key definitions glossary, and key concept outlines.`;
    ...
    console.error('Workspace summary generation failed:', err);
    const errStr = String(err.message || err).toLowerCase();
    const isQuotaError = errStr.includes('quota') || 
                         errStr.includes('rate limit') || 
                         errStr.includes('rate-limit') || 
    ...
    if (isQuotaError) {
      try {
        console.warn('Quota exceeded detected, falling back to simulated summary...');
        const fallbackQuery = query || `__SYNTHESIZE_SUMMARY__ Extract executive summary, key definitions glossary, and key concept outlines.`;
        const fallbackSystemPrompt = systemPrompt || `You are a research synthesis AI.`;
        ...
        warningDiv.style.color = 'var(--text-primary)';
        warningDiv.innerHTML = `<strong>⚠️ Google API Quota Exceeded</strong><br>Your Gemini API key ran out of quota. We have automatically generated a simulated summary briefing offline so you are not blocked.`;
        output.insertBefore(warningDiv, output.firstChild);
        return;
      } catch (mockErr) {
        console.error('Summary fallback simulation failed:', mockErr);
      }
    }
...
async function generatePresentationSlides() {
  ...
  const isVerified = STATE.api.key && STATE.api.isValid;
  if (!isVerified && checkTrialLimitReached('Presentation Slide Outline')) {
    return;
  }
  ...
  try {
    const ragContext = getRAGContext();
    systemPrompt = `You are an educational designer. Format your output strictly using these boundary tags: [SLIDES]...[/SLIDES] and [FAQ]...[/FAQ].
...
${ragContext}`;
    ...
    query = `__BUILD_PRESENTATION__ Build a presentation layout with slides and FAQ study outlines.`;
    ...
    console.error('Presentation outline generation failed:', err);
    const errStr = String(err.message || err).toLowerCase();
    const isQuotaError = errStr.includes('quota') || 
                         errStr.includes('rate limit') || 
                         errStr.includes('rate-limit') || 
    ...
    if (isQuotaError) {
      try {
        console.warn('Quota exceeded detected, falling back to simulated presentation...');
        const fallbackQuery = query || `__BUILD_PRESENTATION__ Build a presentation layout with slides and FAQ study outlines.`;
        const fallbackSystemPrompt = systemPrompt || `You are an educational designer.`;
        
        const mockResult = await simulateMockAIResponse(fallbackQuery, fallbackSystemPrompt, (partialText) => {
          output.innerHTML = `<div style="font-size:0.8rem; color:var(--text-secondary); white-space:pre-wrap;">${escapeHTML(partialText)}</div>`;
        });
        
        renderSlideOutlineResult(mockResult);
        ...
        warningDiv.style.fontSize = '0.75rem';
        warningDiv.style.color = 'var(--text-primary)';
        warningDiv.innerHTML = `<strong>⚠️ Google API Quota Exceeded</strong><br>Your Gemini API key ran out of quota. We have automatically generated a simulated slide deck offline so you are not blocked.`;
        output.insertBefore(warningDiv, output.firstChild);
        return;
      } catch (mockErr) {
        console.error('Presentation fallback simulation failed:', mockErr);
      }
    }
...
function updateUIWorkspace() {
  ...
  // Auto-refresh Mind Map in the background when sources update
  generateInteractiveMindMap(false);
}
...
function updateAPIIndicatorButton() {
  ...
  if (STATE.api.isValid && STATE.api.key) {
  ...
  if (keyInput) keyInput.value = STATE.api.key;
  
  const demoCheckbox = document.getElementById('demo-mode-checkbox');
  if (demoCheckbox) demoCheckbox.checked = STATE.api.demoMode;
  
  const modelSelect = document.getElementById('model-select');
  if (modelSelect) modelSelect.value = STATE.api.model;
  
  const tempSlider = document.getElementById('temperature-slider');
  if (tempSlider) tempSlider.value = STATE.api.temperature;
  
  const tempVal = document.getElementById('temperature-value');
  if (tempVal) tempVal.innerText = STATE.api.temperature;
}
...
async function startupInit() {
  ...
  if (STATE.api.key) {
    validateAPIKeySilently(STATE.api.key).then(isValid => {
      STATE.api.isValid = isValid;
      localStorage.setItem('york_api_key_valid', isValid ? 'true' : 'false');
      updateAPIIndicatorButton();
    });
  } else {
    STATE.api.isValid = false;
    localStorage.setItem('york_api_key_valid', 'false');
    updateAPIIndicatorButton();
  }
  ...
  document.getElementById('save-api-btn').onclick = async () => {
    ...
    let isValid = false;
    if (key) {
      isValid = await validateAPIKeySilently(key);
    }
    
    STATE.api.key = key;
    STATE.api.demoMode = isValid ? false : demo;
    STATE.api.model = model;
    STATE.api.temperature = temp;
    STATE.api.isValid = isValid;
    
    localStorage.setItem('york_gemini_key', key);
    localStorage.setItem('york_demo_mode', isValid ? 'false' : demo.toString());
    ...
    if (key && !isValid) {
      alert('⚠️ The Gemini API key you entered could not be verified. The config button on the top right will remain RED.');
    }
  ...
  document.getElementById('test-api-btn').onclick = async () => {
    ...
    const isValid = await validateAPIKeySilently(key);
    if (isValid) {
      statusText.innerText = 'Connection Valid! Click Apply to Save.';
      statusText.className = 'test-status-message success';
      // Automatically uncheck demo mode when a valid key is tested successfully
      document.getElementById('demo-mode-checkbox').checked = false;
      STATE.api.demoMode = false;
    } else {
